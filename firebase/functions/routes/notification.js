...
router.put('/notifications/rules', isAdmin, validateModel("rule", ["message", "compared", "min", "max", "equation", "attribute"]), (req, res) => {
    db.collection('notification_rules').update(req.body.rule)
    .then((newObject) => {
        return newObject.get();
    })
    .then((doc) => {
